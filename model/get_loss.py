...
def get_tp_losses(outputs, targets, loss_func):
	...
	for group in target_part1:
		loss_g[f"autoencoder_layer{group}"] = loss_func(output_part1[group], target_part1[group])
	output_part2 = outputs["forward"]
	target_part2 = targets["forward"]
	for group in target_part2:
		loss_g[f"forward_layer{group}"] = loss_func(output_part2[group], target_part2[group])
